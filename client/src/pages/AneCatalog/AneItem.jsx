...
const AneItem = ({ images, model, description, pre, tech, price }) => {
  document.title = `${model} | Сенатмед`;
  document.querySelector('meta[name="description"]').content = `${model} ООО "Сенатмед"`;
  document.querySelector('meta[name="keywords"]').content = `${model}, медицина, Анестезиология, нейрореабилитация`;
  const settings = {
    dots: true,
    arrows: false,
    slidesToShow: 1,
